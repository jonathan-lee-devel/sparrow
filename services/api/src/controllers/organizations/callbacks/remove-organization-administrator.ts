...
export const makeRemoveOrganizationAdministratorCallback = (
  ...
  const indexOfAdministratorEmailToRemove = organization.administratorEmails.indexOf(administratorEmailToRemove, 0);
  if (indexOfAdministratorEmailToRemove <= -1) {
    return res.status(HttpStatus.BAD_REQUEST).json({error: `Organization with ID: ${organizationId} does not have administrator: ${administratorEmailToRemove}`});
  }

  organization.administratorEmails.splice(indexOfAdministratorEmailToRemove, 1);
